...
def get_progress_ajax(request, test_id):
    ...
    try:
        ...
        # Add headline details - only alternatives (no original)
        headlines_status = []
        
        # Alternative headlines only
...
def delete_test(request, test_id):
    ...
    if request.method == 'POST':
        test_name = test.original_headline[:50]
        test.delete()
        messages.success(request, f'Test "{test_name}..." has been deleted.')
