...
def map_csv_to_contact(row):
    ...
    for key in ['email', 'e-mail', 'e_mail', 'email_address']:
        if key in normalized_row and normalized_row[key]:
            contact_data['email'] = normalized_row[key]
            break
    
    # First name
    for key in ['first_name', 'firstname', 'first', 'given_name']:
        if key in normalized_row and normalized_row[key]:
            contact_data['first_name'] = normalized_row[key]
            break
    
    # Last name
    for key in ['last_name', 'lastname', 'last', 'surname', 'family_name']:
        if key in normalized_row and normalized_row[key]:
            contact_data['last_name'] = normalized_row[key]
            break
    
    # Organization
    for key in ['organization', 'organisation', 'company', 'company_name']:
        if key in normalized_row and normalized_row[key]:
            contact_data['organization'] = normalized_row[key]
            break
    
    # Job title
    for key in ['job_title', 'jobtitle', 'title', 'position', 'role']:
        if key in normalized_row and normalized_row[key]:
            contact_data['job_title'] = normalized_row[key]
            break
    
    # Phone
    for key in ['phone', 'phone_number', 'telephone', 'mobile', 'cell']:
        if key in normalized_row and normalized_row[key]:
            contact_data['phone'] = normalized_row[key]
            break
    
    # Address
    for key in ['address', 'street', 'street_address']:
        if key in normalized_row and normalized_row[key]:
            contact_data['address'] = normalized_row[key]
            break
    
    # City
    for key in ['city', 'town']:
        if key in normalized_row and normalized_row[key]:
            contact_data['city'] = normalized_row[key]
            break
    
    # State
    for key in ['state', 'province', 'region']:
        if key in normalized_row and normalized_row[key]:
            contact_data['state'] = normalized_row[key]
            break
    
    # Country
    for key in ['country']:
        if key in normalized_row and normalized_row[key]:
            contact_data['country'] = normalized_row[key]
            break
    
    # Postal code
    for key in ['postal_code', 'postalcode', 'zip', 'zip_code', 'postcode']:
        if key in normalized_row and normalized_row[key]:
            contact_data['postal_code'] = normalized_row[key]
            break
    
    # Category
    for key in ['category', 'type', 'group']:
        if key in normalized_row and normalized_row[key]:
            contact_data['category'] = normalized_row[key]
            break
    
    # Tags
    for key in ['tags', 'labels', 'keywords']:
        if key in normalized_row and normalized_row[key]:
            contact_data['tags'] = normalized_row[key]
            break
    
    # Notes
    for key in ['notes', 'note', 'comments', 'description']:
        if key in normalized_row and normalized_row[key]:
            contact_data['notes'] = normalized_row[key]
            break
...
def import_contacts_from_csv(csv_file, skip_duplicates=True, update_existing=False, created_by=None):
    ...
    with transaction.atomic():
        for idx, row in enumerate(rows, start=2):  # Start at 2 (1 is header)
            ...
            # Map to contact data
            contact_data = map_csv_to_contact(row)
